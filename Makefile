...
# Reformat code style
format:
	$(poetry_exec) run black vcore && \
	git add -u && \
	$(poetry_exec) run isort vcore && \
	git add -u && \
	$(poetry_exec) run autoflake \
		--ignore-init-module-imports \
		--remove-unused-variables \
		--recursive \
		--in-place vcore tests && \
	git add -u

# Run tests locally
test:
	$(poetry_exec) run pytest tests --cov=vcore --cov-report=html

# Tests command for CI with .coveragerc report
test-ci:
	$(poetry_exec) run coverage run --source=vcore -m pytest tests
